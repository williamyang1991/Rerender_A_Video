...
def poisson_fusion(blendI, I1, I2, mask, grad_weight=[2.5,0.5,0.5], updateA=True):
    global As

    Iab = cv2.cvtColor(blendI, cv2.COLOR_RGB2LAB).astype(float)
    Ia = cv2.cvtColor(I1, cv2.COLOR_RGB2LAB).astype(float)
    Ib = cv2.cvtColor(I2, cv2.COLOR_RGB2LAB).astype(float)
    m = (mask > 127).astype(float)
    h, w, c = Iab.shape
        ...
        for i in range(h):
            for j in range(w):
                ...
                if j < w-1:
                    ...
                    indgy_y += [i * w + j + 1]
                    vdy += [-1]
        Ix = scipy.sparse.coo_array((np.ones(h*w), (np.arange(h*w), np.arange(h*w))), shape=(h*w, h*w)).tocsc()
        Gx = scipy.sparse.coo_array((np.array(vdx), (np.array(indgx_x), np.array(indgx_y))), shape=(h*w, h*w)).tocsc()
        Gy = scipy.sparse.coo_array((np.array(vdy), (np.array(indgy_x), np.array(indgy_y))), shape=(h*w, h*w)).tocsc()
        As = []
        for i in range(3):
            As += [scipy.sparse.vstack([Gx * grad_weight[i], Gy * grad_weight[i], Ix])]
    ...
    return cv2.cvtColor(final.astype(np.uint8), cv2.COLOR_LAB2RGB)
